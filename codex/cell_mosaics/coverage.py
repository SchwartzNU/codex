...
try:
    ...
    polygon = None  # fallback implemented below
...
class CoverageDensityMapper:
    ...
    def _rasterize(self, pts: np.ndarray) -> None:
        ...
        if polygon is not None:
            ...
            # Fallback rasterization via matplotlib.path.Path
            from matplotlib.path import Path as _MPath
            import numpy as _np
            poly = _np.vstack([x, y]).T
            path = _MPath(poly)
            yy, xx = _np.mgrid[0:self.coverage_count.shape[0], 0:self.coverage_count.shape[1]]
            pts = _np.vstack([xx.ravel(), yy.ravel()]).T
            mask = path.contains_points(pts)
            mask = mask.reshape(self.coverage_count.shape)
            self.coverage_count[mask] += 1
